...
def do_train():
    ...
    log.backup_log_and_create_new()

    log_message("INFO", "cli.train.do_train: Parsing command-line arguments for data, training, model, and LoRa configurations...")
    ...
    log_message("INFO", f"cli.train.do_train: Configuring logging to file: {os.path.join(args.output_dir, 'config.log')}")
    logging_config(os.path.join(args.output_dir, "config.log"))
   

    if args.token:
        log_message("INFO", f"Logging into the Hugging Face Hub with token {args.token[:10]}...")
        logger.info(f"Logging into the Hugging Face Hub with token {args.token[:10]}...")
        login(token=args.token)
    
    log_message("INFO", f"cli.train.do_train: Saving package versions to: {os.path.join(args.output_dir, 'requirements.txt')}")
    # save_package_versions(os.path.join(args.output_dir, "requirements.txt"))
    
    log_message("INFO", "cli.train.do_train: Calling blm.utils.train to start the model training process...")
    train(args)
